\documentclass[11pt]{article}

\usepackage[a4paper]{geometry}
\geometry{margin=2cm}
\usepackage[tiny,compact]{titlesec}
\usepackage{fullpage}
\usepackage{graphicx}
\usepackage{wrapfig}

\usepackage[T1]{fontenc}
\usepackage{fontspec}
\usepackage{xltxtra,xunicode}
\defaultfontfeatures{Mapping=tex-text,Scale=MatchUppercase}
\newcommand\euro[1]{€#1}
\newcommand\ukp[1]{£#1}
\renewcommand{\familydefault}{\rmdefault}
\setmainfont{Times New Roman}
\setmonofont{Hack}

\usepackage[unicode=true]{hyperref}
\hypersetup{breaklinks=true,
  bookmarks=true,
  pdfauthor={},
  pdftitle={},
  colorlinks=true,
  urlcolor=blue,
  linkcolor=blue,
  pdfborder={0 0 0}}
\newcommand\fnurl[2]{%
  \href{#1}{#2}%
}

\usepackage{fancyhdr,lastpage}
\renewcommand{\headrulewidth}{0.4pt}
\setlength{\headheight}{20pt}
\pagestyle{fancy}
\lhead{Dr.~Richard Mortier: Publications}
\chead{}
\rhead{}
\lfoot{\today}
\cfoot{}
\rfoot{\thepage/\pageref{LastPage}}

\fancypagestyle{firststyle}
{
  \fancyhf{}
  \rfoot{\thepage/\pageref{LastPage}}
}


%% include the actual bibliographies
\usepackage[
backend=biber,
natbib=true,
%%
style=numeric,
sorting=ydnt,
minnames=7,
maxnames=7,
firstinits=true,
defernumbers=true,
%%
url=true,
doi=true,
eprint=false,
isbn=false
]{biblatex}

\addbibresource{strings.bib}
\addbibresource{rmm-journal.bib}
\addbibresource{rmm-conference.bib}
\addbibresource{rmm-workshop.bib}
\addbibresource{rmm-patent.bib}
\addbibresource{rmm-techreport.bib}
\addbibresource{rmm-unpublished.bib}

\DeclareSourcemap{
  \maps[datatype=bibtex]{
    \map{
      \perdatasource{rmm-journal.bib}
      \step[fieldset=keywords, fieldvalue={, journal}, append]
    }
    \map{
      \perdatasource{rmm-conference.bib}
      \step[fieldset=keywords, fieldvalue={, conference}, append]
    }
    \map{
      \perdatasource{rmm-workshop.bib}
      \step[fieldset=keywords, fieldvalue={, workshop}, append]
    }
    \map{
      \perdatasource{rmm-patent.bib}
      \step[fieldset=keywords, fieldvalue={, patent}, append]
    }
    \map{
      \perdatasource{rmm-techreport.bib}
      \step[fieldset=keywords, fieldvalue={, techreport}, append]
    }
  }
}

\begin{document}
\thispagestyle{firststyle}
\part*{\Large Dr.~Richard Mortier, Publications}

{
  \nocite{*}

  \printbibliography[keyword=journal, prefixnumbers=J, title={Journal Papers}]
  \printbibliography[keyword=conference, prefixnumbers=C, title={Conference Papers}]
  \printbibliography[keyword=workshop, prefixnumbers=W, title={Workshop Papers}]
  \printbibliography[keyword=patent, prefixnumbers=P, title={Patents}]
  \printbibliography[keyword=techreport, prefixnumbers=T, title={Technical Reports}]
  \printbibliography[keyword=unpublished, prefixnumbers=U, title={Posters, Book Chapters, etc.}]
}

\end{document}
